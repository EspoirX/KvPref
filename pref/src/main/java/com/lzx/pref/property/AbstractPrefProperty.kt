...
abstract class AbstractPrefProperty<T : Any?> :
    ReadWriteProperty<KvPrefModel, T>, PreferenceProperty, DynamicKeyPref<T> {
    ...
    override operator fun getValue(thisRef: KvPrefModel, property: KProperty<*>): T {
        if (!thisRef.isInTransaction) {
            return getPreference(thisRef,"")
        }
        if (lastUpdate < thisRef.transactionStartTime) {
            transactionData = getPreference(thisRef, "")
    ...
    override operator fun setValue(thisRef: KvPrefModel, property: KProperty<*>, value: T) {
        if (thisRef.isInTransaction) {
            transactionData = value
            lastUpdate = SystemClock.uptimeMillis()
            setEditor(thisRef, "", value)
        } else {
            setPreference(thisRef, "", value)
        }
    }
